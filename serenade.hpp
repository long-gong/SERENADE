...
protected:
  ...
  std::vector <std::vector<KnowledgeSet *>> _B;
  std::vector <std::vector<KnowledgeSet>> _receive;
public:
  SERENADE():  _sg(dummy_perm), _sr(dummy_perm), _port_num(-1), _max_iter(-1) {}
  std::tuple<std::vector<int> /* decisions */, std::vector<int> /* # of iterations */, std::vector<int> /* cycle types */, int /* # of non-ouroboros */> emulate(perm_t &sr, perm_t &sg, matrix_t &Q);
  std::tuple<std::vector<int> /* decisions */, std::vector<int> /* which cycle each vertex is on */, std::vector<int> /* cycle lengths */ > run(perm_t &sr, perm_t &sg, matrix_t &Q);
private:
  void init(perm_t &sr, perm_t &sg);
  void knowledge_disc_zero(int i, matrix_t &Q);
 ...
 * Initialization
 */
void SERENADE::init(std::vector<int> &sr, std::vector<int> &sg) {
  _sg = sg;
  _sr = sr;
  if ( _port_num != sr.size() ) {
    ...
    _B.resize(_port_num, std::vector<KnowledgeSet *>(_port_num, nullptr));
  ...
  for ( int i = 0;i < _port_num;++ i ) {
    ...
    std::fill(_B[i].begin(), _B[i].end(), nullptr);
  }

  for (int i = 0; i < _port_num; ++i) {
    auto i_prime = sg[sr[i]];
    _sigma[i] = i_prime;
...
std::tuple<std::vector<int> /* decisions */, std::vector<int> /* # of iterations */, std::vector<int> /* cycle types */, int /* # of non-ouroboros */>
    SERENADE::emulate(SERENADE::perm_t &sr, SERENADE::perm_t &sg, SERENADE::matrix_t &Q) {

  init(sr, sg);
  for (int i = 0; i < _port_num; ++i) knowledge_disc_zero(i, Q);

  std::vector<int> iterations(_port_num, 1);
  std::vector<int> type_of_cycles(_port_num, 1);
  for (int k = 1; k <= _max_iter; ++k) {
    for (int i = 0; i < _port_num; ++i) {
      if (_decisions[i] == -1) {
 ...
 */
void SERENADE::knowledge_disc_zero(int i, SERENADE::matrix_t &Q) {
  auto i_d = _sigma[i];
...
void SERENADE::knowledge_disc_halt_check(int i, int k) {
  ...
  if (_B[i][i_d] != nullptr) {
    // rediscover
    if (_B[i][i_d]->leader == -1) {
      // same direction
      _decisions[i] = ((ks1.wr - _B[i][i_d]->wr) > (ks1.wg - _B[i][i_d]->wg) ? 1 : 0);
    } else {
      // opposite direction
      _decisions[i] = ((ks1.wr + _B[i][i_d]->wr) > (ks1.wg + _B[i][i_d]->wg) ? 1 : 0);
    }
  ...
  if (_B[i][i_u] != nullptr) {
    // rediscover
    if (_B[i][i_u]->leader != -1) {
      // same direction
      _decisions[i] = ((ks2.wr - _B[i][i_u]->wr) > (ks2.wg - _B[i][i_u]->wg) ? 1 : 0);
    } else {
      // opposite direction
      _decisions[i] = ((ks2.wr + _B[i][i_u]->wr) > (ks2.wg + _B[i][i_u]->wg) ? 1 : 0);
    }
...
std::tuple<std::vector<int> /* decisions */,
           ...
           std::vector<int> /* cycle lengths */ > SERENADE::run(SERENADE::perm_t &sr,
                                                                SERENADE::perm_t &sg,
                                                                SERENADE::matrix_t &Q) {
  auto N = sg.size();
  ...
  return {decisions, cycles, cycle_lens};
}
